...
namespace zdunk
{
    ...
    public:
        ...
        TcpServer(IOManager *woker = IOManager::GetThis(), IOManager *io_worker = IOManager::GetThis(), IOManager *awoker = IOManager::GetThis());
        virtual ~TcpServer();

        virtual bool bind(Address::ptr addr, bool ssl = false);
        virtual bool bind(std::vector<Address::ptr> addr, std::vector<Address::ptr> &fails, bool ssl = false);
