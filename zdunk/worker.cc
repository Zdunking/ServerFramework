...
namespace zdunk
{
    ...
    static zdunk::ConfigVar<std::map<std::string, std::map<std::string, std::string>>>::ptr g_worker_config = zdunk::Config::Lookup("workers", std::map<std::string, std::map<std::string, std::string>>(), "worker config");

    WorkerGroup::WorkerGroup(uint32_t batch_size, zdunk::Scheduler *s)
    ...
    bool WorkerManager::init(const std::map<std::string, std::map<std::string, std::string>> &v)
    {
        for (auto &i : v)
        {
            ...
            int32_t thread_num = zdunk::GetParamValue(i.second, "thread_num", 1);
            int32_t worker_num = zdunk::GetParamValue(i.second, "worker_num", 1);
