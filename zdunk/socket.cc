...
namespace zdunk
{
    ...
    Socket::ptr Socket::CreateTCP(zdunk::Address::ptr address)
    {
        Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
        return sock;
    }

    Socket::ptr Socket::CreateUDP(zdunk::Address::ptr address)
    {
    ...
    bool Socket::bind(const Address::ptr addr)
    {
        ...
        {
            newSock();
            if (ZDUNK_UNLIKELY(!isValid()))
            {
                return false;
            }
        }

        if (ZDUNK_UNLIKELY(addr->getFamily() != m_family))
        {
    ...
    int Socket::recvFrom(void *buffer, size_t length, const Address::ptr from, int flags /* = 0*/)
    {
        if (isConnected())
        {
    ...
    int Socket::recvFrom(iovec *buffers, size_t length, const Address::ptr from, int flags /* = 0*/)
    {
        if (isConnected())
        {
            msghdr msg;
    ...
    SSLSocket::ptr SSLSocket::CreateTCP(zdunk::Address::ptr address)
    {
