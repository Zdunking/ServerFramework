...
namespace zdunk
{
    ...
    ByteArray::ByteArray(size_t base_size)
        : m_baseSize(base_size), m_position(0), m_capacity(base_size), m_size(0), m_endian(ZDUNK_BIG_ENDIAN), m_root(new Node(base_size)), m_cur(m_root)
    {
    }
    ...
    {
        return m_endian == ZDUNK_LITTLE_ENDIAN;
    }
    ...
    void ByteArray::setIsLittleEndian(bool val)
    {
        if (val)
        {
            m_endian = ZDUNK_LITTLE_ENDIAN;
        }
        else
        {
            m_endian = ZDUNK_BIG_ENDIAN;
        }
    }
