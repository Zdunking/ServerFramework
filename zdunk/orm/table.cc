...
namespace zdunk
{
    namespace orm
    {
        ...
        static zdunk::Logger::ptr g_logger = LOG_NAME("orm");

        std::string Table::getFilename() const
        {
            return zdunk::ToLower(m_name + m_subfix);
        }
        ...
        {
            std::string p = path + "/" + zdunk::replace(m_namespace, ".", "/");
            zdunk::FSUtil::Mkdir(p);
            gen_inc(p);
            gen_src(p);
        }
        ...
        void Table::gen_inc(const std::string &path)
        {
            ...
            std::set<std::string> incs = {"zdunk/db/db.h", "zdunk/util.h"};
            for (auto &i : incs)
            {
                ofs << "#include \"" << i << "\"" << std::endl;
            }
            ...
            std::vector<std::string> ns = zdunk::split(m_namespace, '.');
        ...
        std::string Table::genToStringSrc(const std::string &class_name)
        {
                 ...
                 it != m_cols.end(); ++it)
            {
                ...
                {
                    ss << "zdunk::Time2Str(" << GetAsMemberName((*it)->getName()) << ")"
                       << ";" << std::endl;
                }
                else
                {
            ...
            ss << "    return zdunk::JsonUtil::ToString(jvalue);" << std::endl;
            ss << "}" << std::endl;
            return ss.str();
        }
        ...
        void Table::gen_src(const std::string &path)
        {
            ...
            ofs << "#include \"zdunk/log.h\"" << std::endl;
            ofs << std::endl;
            ...
            ofs << std::endl;
            ofs << "static zdunk::Logger::ptr g_logger = LOG_NAME(\"orm\");" << std::endl;
