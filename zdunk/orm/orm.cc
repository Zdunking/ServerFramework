...
void gen_cmake(const std::string &path, const std::map<std::string, zdunk::orm::Table::ptr> &tbs)
{
    ...
    for (auto &i : tbs)
    {
        ofs << "    " << zdunk::replace(i.second->getNamespace(), ".", "/")
            << "/" << zdunk::ToLower(i.second->getFilename()) << ".cc" << std::endl;
    }
    ofs << ")" << std::endl;
...
int main(int argc, char **argv)
{
    ...
    zdunk::FSUtil::ListAllFile(files, input_path, ".xml");
    std::vector<zdunk::orm::Table::ptr> tbs;
    ...
    for (auto &i : files)
    {
        ...
        {
            zdunk::orm::Table::ptr table(new zdunk::orm::Table);
    ...
    std::map<std::string, zdunk::orm::Table::ptr> orm_tbs;
    for (auto &i : tbs)
    {
        i->gen(out_path);
        orm_tbs[i->getName()] = i;
