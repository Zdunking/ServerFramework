...
namespace zdunk
{
    ...
    static zdunk::ConfigVar<std::map<std::string, std::map<std::string, std::string>>>::ptr g_thread_info_set = Config::Lookup("fox_thread", std::map<std::string, std::map<std::string, std::string>>(), "confg for thread");
    ...
    void FoxThread::read_cb(evutil_socket_t sock, short which, void *args)
    {
        ...
        if (recv(sock, cmd, sizeof(cmd), 0) > 0)
        {
            ...
            zdunk::Atomic::addFetch(thread->m_total, callbacks.size());
    ...
    bool FoxThreadPool::dispatch(callback cb)
    {
        do
        {
            zdunk::Atomic::addFetch(m_total, (uint64_t)1);
            RWMutex::WriteLock lock(m_mutex);
            if (!m_advance)
            {
    ...
    {
        zdunk::Atomic::addFetch(m_total, cbs.size());
        RWMutex::WriteLock lock(m_mutex);
        if (!m_advance)
        {
            for (auto cb : cbs)
    ...
    const std::string &FoxThread::GetFoxThreadName()
    {
        ...
        uint64_t tid = zdunk::GetThreadId();
        do
        {
    ...
    {
        m_name = m_name + "_" + std::to_string(zdunk::GetThreadId());
        s_thread = this;

        RWMutex::WriteLock lock(s_thread_mutex);
        s_thread_names[zdunk::GetThreadId()] = m_name;
    }
    ...
    void FoxThread::unsetThis()
    {
        s_thread = nullptr;
        RWMutex::WriteLock lock(s_thread_mutex);
        s_thread_names.erase(zdunk::GetThreadId());
    }
    ...
    void FoxThreadManager::init()
    {
        ...
        for (auto i : m)
        {
            auto num = zdunk::GetParamValue(i.second, "num", 0);
            auto name = i.first;
            auto advance = zdunk::GetParamValue(i.second, "advance", 0);
            if (num <= 0)
            {
