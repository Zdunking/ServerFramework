...
namespace zdunk
{
    ...
    static zdunk::ConfigVar<std::map<std::string, std::map<std::string, std::string>>>::ptr g_sqlite3_dbs = zdunk::Config::Lookup("sqlite3.dbs", std::map<std::string, std::map<std::string, std::string>>(), "sqlite3 dbs");
    ...
    {
        return bind(idx, zdunk::Time2Str(value));
    }
    ...
    time_t SQLite3Data::getTime(int idx)
    {
        ...
        return zdunk::Str2Time(str.c_str());
    }
    ...
    SQLite3::ptr SQLite3Manager::get(const std::string &name)
    {
        ...
        lock.unlock();
        std::string path = zdunk::GetParamValue<std::string>(args, "path");
        if (path.empty())
        {
        ...
        {
            path = zdunk::EnvMgr::GetInstance()->getAbsoluteWorkPath(path);
        }
        ...
        std::string sql = zdunk::GetParamValue<std::string>(args, "sql");
        if (!sql.empty())
        {
            if (rt->execute(sql))
            {
