...
namespace zdunk
{
    ...
    static zdunk::ConfigVar<std::map<std::string, std::map<std::string, std::string>>>::ptr g_mysql_dbs = zdunk::Config::Lookup("mysql.dbs", std::map<std::string, std::map<std::string, std::string>>(), "mysql dbs");
                             ...
                             const int &timeout)
    {
        ...
        int port = zdunk::GetParamValue(params, "port", 0);
        std::string host = zdunk::GetParamValue<std::string>(params, "host");
        std::string user = zdunk::GetParamValue<std::string>(params, "user");
        std::string passwd = zdunk::GetParamValue<std::string>(params, "passwd");
        std::string dbname = zdunk::GetParamValue<std::string>(params, "dbname");
    ...
    bool MySQL::connect()
    {
        ...
        m_hasError = false;
        m_poolSize = zdunk::GetParamValue(m_params, "pool", 5);
        m_mysql.reset(m, mysql_close);
        return true;
    }

    zdunk::IStmt::ptr MySQL::prepare(const std::string &sql)
    {
    ...
    {
        m_cmd = zdunk::StringUtil::Formatv(format, ap);
    ...
    {
        return MySQL::ptr(this, zdunk::nop<MySQL>);
    }
...
#define BIND_COPY_LEN(ptr, size)                                \
    ...
    int MySQLStmt::bindTime(int idx, const time_t &value)
    {
        ...
        // return 0;
        return bindString(idx, zdunk::Time2Str(value));
    }
    ...
    {
        return zdunk::TypeUtil::Atoi(m_cur[idx]);
    }
    ...
    {
        return zdunk::TypeUtil::Atof(m_cur[idx]);
    }
    ...
    time_t MySQLRes::getTime(int idx)
    {
        ...
        return zdunk::Str2Time(m_cur[idx]);
    }
