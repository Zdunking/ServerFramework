...
namespace zdunk
{
    ...
    static zdunk::ConfigVar<std::map<std::string, std::map<std::string, std::string>>>::ptr g_redis =
        zdunk::Config::Lookup("redis.config", std::map<std::string, std::map<std::string, std::string>>(), "redis config");
    ...
    Redis::Redis(const std::map<std::string, std::string> &conf)
    {
        ...
        m_port = zdunk::TypeUtil::Atoi(tmp.substr(pos + 1));
        m_passwd = get_value(conf, "passwd");
        m_logEnable = zdunk::TypeUtil::Atoi(get_value(conf, "log_enable", "1"));
        ...
        uint64_t v = zdunk::TypeUtil::Atoi(tmp);
    ...
    bool Redis::connect(const std::string &ip, int port, uint64_t ms)
    {
        ...
        if (c)
        {
            ...
            if (!m_passwd.empty())
            {
                ...
                if (!r)
                {
                    LOG_ERROR(g_logger) << "auth error:("
                                        << m_host << ":" << m_port << ", " << m_name << ")";
                    return false;
                }
                if (r->type != REDIS_REPLY_STATUS)
                {
                    LOG_ERROR(g_logger) << "auth reply type error:" << r->type << "("
                                        << m_host << ":" << m_port << ", " << m_name << ")";
                    return false;
                }
                if (!r->str)
                {
                    LOG_ERROR(g_logger) << "auth reply str error: NULL("
                                        << m_host << ":" << m_port << ", " << m_name << ")";
                    return false;
                }
                ...
                {
                    LOG_ERROR(g_logger) << "auth error: " << r->str << "("
                                        << m_host << ":" << m_port << ", " << m_name << ")";
                    return false;
                }
            }
    ...
    FoxRedis::FoxRedis(zdunk::FoxThread *thr, const std::map<std::string, std::string> &conf)
    ...
    void FoxRedis::OnAuthCb(redisAsyncContext *c, void *rp, void *priv)
    {
        ...
        if (strcmp(r->str, "OK") == 0)
        {
            LOG_INFO(g_logger) << "auth ok: " << r->str << "("
                               << fr->m_host << ":" << fr->m_port << ", " << fr->m_name << ")";
        }
        else
        {
    ...
    void FoxRedis::ConnectCb(const redisAsyncContext *c, int status)
    {
        ...
        if (!status)
        {
            ...
            if (!ar->m_passwd.empty())
            {
                ...
                if (rt)
                {
                    LOG_ERROR(g_logger) << "FoxRedis Auth fail: " << rt;
                }
            }
        }
        else
    ...
    bool FoxRedis::init()
    {
        if (m_thread == zdunk::FoxThread::GetThis())
        {
            return pinit();
        }
        else
    ...
    bool FoxRedis::pinit()
    {
        ...
        m_context.reset(ctx, zdunk::nop<redisAsyncContext>);
    ...
    ReplyPtr FoxRedis::cmd(const char *fmt, va_list ap)
    {
        ...
        //     free(buf);
        // }
        // ctx->scheduler = zdunk::Scheduler::GetThis();
        // ctx->fiber = zdunk::Fiber::GetThis();
        // ctx->thread = m_thread;

        FCtx fctx;
        fctx.cmd.append(buf, len);
        free(buf);
        fctx.scheduler = zdunk::Scheduler::GetThis();
        fctx.fiber = zdunk::Fiber::GetThis();
    ...
    FoxRedis::Ctx::Ctx(FoxRedis *r)
        : ev(nullptr), timeout(false), rds(r)
          ...
          thread(nullptr)
    {
        zdunk::Atomic::addFetch(rds->m_ctxCount, 1);
    }

    FoxRedis::Ctx::~Ctx()
    {
        // cancelEvent();
        ZDUNK_ASSERT(thread == zdunk::FoxThread::GetThis());
        // ZDUNK_ASSERT(destory == 0);
        zdunk::Atomic::subFetch(rds->m_ctxCount, 1);
        //++destory;
        // cancelEvent();
        if (ev)
        {
    ...
    {
        // if(ev) {
        //     if(thread == zdunk::FoxThread::GetThis()) {
        //         evtimer_del(ev);
        //         event_free(ev);
        //     } else {
        //         auto e = ev;
    ...
    FoxRedisCluster::FoxRedisCluster(zdunk::FoxThread *thr, const std::map<std::string, std::string> &conf)
        : m_thread(thr), m_status(UNCONNECTED), m_event(nullptr)
    {
        m_ctxCount = 0;
    ...
    void FoxRedisCluster::ConnectCb(const redisAsyncContext *c, int status)
    {
        ...
        if (!status)
        {
            ...
            if (!ar->m_passwd.empty())
            {
                ...
                if (rt)
                {
                    LOG_ERROR(g_logger) << "FoxRedisCluster Auth fail: " << rt;
                }
            }
        }
        else
    ...
    bool FoxRedisCluster::pinit()
    {
        ...
        m_status = CONNECTED;
        // m_context.reset(ctx, redisAsyncFree);
        m_context.reset(ctx, zdunk::nop<redisClusterAsyncContext>);
    ...
    {
        // LOG_INFO(g_logger) << "cancelEvent " << zdunk::FoxThread::GetThis()
        //            << " - " << thread
        //            << " - " << zdunk::IOManager::GetThis()
        //            << " - " << cancel_count;
        // if(thread != zdunk::FoxThread::GetThis()) {
        //     LOG_INFO(g_logger) << "cancelEvent " << zdunk::FoxThread::GetThis()
        //                << " - " << thread
        ...
        //    //           << " this=" << zdunk::FoxThread::GetThis();
        //    //ZDUNK_ASSERT(thread == zdunk::FoxThread::GetThis());
        //}
        // ZDUNK_ASSERT(!zdunk::IOManager::GetThis());
        ////if(zdunk::Atomic::addFetch(cancel_count) > 1) {
        ////    return;
        ////}
        ////ZDUNK_ASSERT(!zdunk::Fiber::GetThis());
        ////zdunk::RWMutex::WriteLock lock(mutex);
        // if(++cancel_count > 1) {
        //     return;
        // }
        // if(ev) {
    ...
    {
        zdunk::RWMutex::WriteLock lock(m_mutex);
        ...
        if (r->getType() == IRedis::FOX_REDIS || r->getType() == IRedis::FOX_REDIS_CLUSTER)
        {
            ...
            return std::shared_ptr<IRedis>(r, zdunk::nop<IRedis>);
        }
        lock.unlock();
    ...
    void RedisManager::init()
    {
        ...
        for (auto &i : m_config)
        {
            ...
            auto pool = zdunk::TypeUtil::Atoi(get_value(i.second, "pool"));
            ...
            for (int n = 0; n < pool; ++n)
            {
                if (type == "redis")
                {
                    zdunk::Redis *rds(new zdunk::Redis(i.second));
                    rds->connect();
                    rds->setLastActiveTime(time(0));
                    zdunk::RWMutex::WriteLock lock(m_mutex);
                    m_datas[i.first].push_back(rds);
                    zdunk::Atomic::addFetch(done, 1);
                }
                else if (type == "redis_cluster")
                {
                    zdunk::RedisCluster *rds(new zdunk::RedisCluster(i.second));
                    rds->connect();
                    rds->setLastActiveTime(time(0));
                    zdunk::RWMutex::WriteLock lock(m_mutex);
                    m_datas[i.first].push_back(rds);
                ...
                else if (type == "fox_redis")
                {
                    auto conf = i.second;
                    auto name = i.first;
                    zdunk::FoxThreadMgr::GetInstance()->dispatch("redis", [this, conf, name, &done]()
                                                                 {
                    zdunk::FoxRedis* rds(new zdunk::FoxRedis(zdunk::FoxThread::GetThis(), conf));
                    rds->init();
                    rds->setName(name);
                ...
                else if (type == "fox_redis_cluster")
                {
                    ...
                    zdunk::FoxThreadMgr::GetInstance()->dispatch("redis", [this, conf, name, &done]()
                                                                 {
                    zdunk::FoxRedisCluster* rds(new zdunk::FoxRedisCluster(zdunk::FoxThread::GetThis(), conf));
                    rds->init();
                    rds->setName(name);
