    ...
    namespace http
    {
        ...
        HttpResult::ptr HttpConnection::DoRequest(HttpMethod method, Uri::ptr uri, uint64_t timeout_ms, const std::map<std::string, std::string> &headers /* = {}*/, const std::string &body /* = ""*/)
        {
            ...
            for (auto &i : headers)
            {
                ...
                {
                    if (strcasecmp(i.second.c_str(), "keep-alived") == 0)
                    {
                        req->setClose(false);
                        continue;
                    }
        ...
        {
            uint64_t now_ms = zdunk::GetCurrentMS();
        ...
        {
            ++ptr->m_request;
            if (!ptr->isConnected() || ((ptr->m_createTime + pool->m_maxAliveTime) >= zdunk::GetCurrentMS()) || (ptr->m_request >= pool->m_maxRequest))
            {
                delete ptr;
                --pool->m_total;
                return;
